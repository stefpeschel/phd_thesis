...
In Chapter~\ref{ch:introduction}, we introduced different analysis targets in microbiome studies, distinguishing between community-level, feature-level, and network-level approaches. In this chapter, we focus on \emph{community-level analyses}, where the microbial community is summarized, compared, or modeled as a whole rather than by studying individual taxa separately. Such approaches are often used as a first step in microbiome data analysis because they provide interpretable summaries of global community structure and allow researchers to assess whether microbial communities differ across samples, groups, or environmental conditions.

Community-level analyses can take several forms. Within-sample diversity measures, commonly referred to as alpha diversity, reduce each sample to a scalar quantity that describes aspects such as richness, entropy, or dominance of the microbial community. Between-sample dissimilarities, often discussed under the term beta diversity, compare pairs of samples and summarize the relational structure of the entire sample collection through a distance or dissimilarity matrix. These dissimilarities form the basis for ordination methods, distance-based tests such as PERMANOVA, and procedures for assessing differences in community dispersion. A further class of approaches models the microbial composition directly as a multivariate response after transformation to an appropriate coordinate system. Such regression models provide a complementary view to distance-based testing because they relate covariates to the transformed community profile itself rather than to a derived dissimilarity matrix. Finally, global equivalence approaches ask whether differences between groups are sufficiently small to be considered negligible, thereby shifting the inferential focus from detecting any difference to assessing practical similarity.

The statistical analysis of such summaries and models requires careful consideration of the structural characteristics discussed in Chapter~\ref{ch:microbiome}, including compositionality, sparsity, and variability in sequencing depth. Rather than assuming a single preprocessing pipeline for all methods, we adopt a method-specific perspective. Some approaches, such as model-based diversity estimation, are designed to account for unequal sampling depth and uncertainty due to unobserved taxa and can therefore be applied directly to count data. Other approaches rely on transformations, in particular log-ratio representations, to obtain a data representation compatible with the geometry of compositional data. This distinction is important because the choice of preprocessing determines the statistical object being analyzed and therefore affects the interpretation of the resulting community-level summaries and regression coefficients.

In this chapter, we concentrate on four classes of methods. First, we introduce within-sample diversity measures, including richness, Shannon diversity, and Gini--Simpson diversity, together with approaches for group comparison and regression modeling of alpha diversity. These methods are widely used in microbiome studies and provide a natural scalar entry point into community-level analysis. Second, we discuss between-sample dissimilarities, focusing on Bray--Curtis dissimilarity and Euclidean distance after centered log-ratio transformation. These summaries are central to exploratory ordination, PERMANOVA, and the analysis of group differences in community dispersion. Third, we consider regression models for compositional responses, where the transformed microbial composition is modeled as a multivariate outcome and covariate effects are tested jointly across taxa. Fourth, we introduce compositional equivalence testing as a complementary inferential perspective that asks whether group differences in microbial composition are sufficiently small to be considered negligible rather than merely testing for any detectable difference.

Together, these approaches provide complementary views of community-level structure. Within-sample diversity summarizes each sample individually, between-sample dissimilarities describe how samples relate to one another, compositional-response regression models covariate effects on the transformed community profile directly, and compositional equivalence provides a global assessment of whether group-level compositional differences are small enough to be regarded as practically negligible. Together, these approaches form the community-level part of the analysis pipeline used in Chapter~\ref{ch:application}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/community/overview_alpha_diversity.png}
\caption{\textbf{Overview of within-sample diversity analysis.} A microbiome count matrix with samples in rows and taxa in columns is summarized across taxa within each sample, yielding one diversity value per sample. These sample-level summaries, commonly referred to as alpha diversity measures, can then be compared between groups or modeled as response variables in regression analyses.}
...
Richness is the most fundamental measure of alpha diversity and is defined as the number of distinct taxa present in a sample. In the context of microbiome data, richness aims to quantify how many microbial features, such as amplicon sequence variants (ASVs), are represented in a community. Formally, for a sample $k$, the observed richness is given by
\begin{equation}
\alpha_{\mathrm{rich}, k}^{\mathrm{obs}} = \sum_{i=1}^p \mathbb{I}(x_{ki} > 0),
\end{equation}
where $\mathbb{I}(\cdot)$ denotes the indicator function. However, this observed number of taxa is typically a biased estimate of the true richness due to finite sampling depth and the presence of unobserved, low-abundance taxa.
...
An alternative approach is proposed by \citet{willis2015estimating}, who estimate the number of unobserved taxa rather than reducing the observed data through subsampling. In particular, we consider richness estimation via the \texttt{breakaway} framework and entropy-based diversity measures estimated with \texttt{DivNet}. These methods provide statistically principled estimates and associated uncertainty measures, enabling valid comparisons of diversity across samples and groups. Conceptually, this shifts the treatment of heterogeneous sampling depth from a preprocessing step to an explicit component of the statistical model, where uncertainty due to incomplete sampling is directly accounted for in the estimation.
...
\end{equation}
where $n_0^{(k)}$ denotes the number of unobserved taxa. The estimation problem therefore reduces to predicting $n_0^{(k)}$.
...
To address these limitations, \citet{willis2018divnet} propose the \texttt{DivNet} framework, which estimates diversity indices based on a model for the underlying compositional structure of the data. The methodology is implemented in the \texttt{DivNet} R package, which provides functions for model fitting, diversity estimation, and uncertainty quantification. Rather than treating samples independently, \texttt{DivNet} models the log-ratios of relative abundances across all samples using a multivariate normal distribution, thereby capturing dependence between taxa and borrowing strength across samples.
...
As an alternative, permutation-based tests provide a flexible framework that does not rely on distributional assumptions. Figure~\ref{fig:3community:overview_alpha_permutation} visualizes a typical permutation testing approach. Let $\hat{\alpha}_k$ denote a diversity estimate for sample $k$, and let $g_k \in \{1,\dots,G\}$ denote the corresponding group label. A test statistic $T = T(\{\hat{\alpha}_k, g_k\}_{k=1}^n)$ is computed for the observed data, for example the Kruskal-Wallis statistic or a difference in group means or medians. Under the null hypothesis of no group effect, the labels $g_k$ are exchangeable, and the null distribution of $T$ can be approximated by repeatedly permuting the group labels and recomputing the statistic. The corresponding empirical $p$-value is given by
\begin{equation}
...
As discussed in Chapter~\ref{ch:permapprox}, empirical $p$-values lie on a coarse grid when only a limited number of permutations $B$ can be performed. In the present setting, this limitation is typically not critical, as alpha diversity analysis usually involves only one or a small number of tests, for example when comparing a single diversity measure across a few groups, so that very small $p$-values are rarely required. In contrast to high-dimensional settings with many simultaneous tests, the coarse resolution of empirical $p$-values therefore has limited impact on inference. However, if a finer resolution of $p$-values is desired without substantially increasing the number of permutations, the empirical permutation distribution can be refined by tail approximation methods such as the approach implemented in \texttt{permApprox} (Chapter~\ref{ch:permapprox}).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/community/overview_alpha_diversity_permutation.png}
\textbf{Permutation principle for alpha diversity comparisons.} Alpha diversity indices are first computed for each sample and combined with the corresponding group labels. To approximate the null distribution of the test statistic, the group labels are randomly permuted while the observed alpha diversity values remain fixed, thereby breaking any association between diversity and group membership. Repeating this procedure yields a permutation distribution against which the observed group difference can be compared.
\label{fig:3community:overview_alpha_permutation}
\end{figure}
...
\end{equation}
where $\boldsymbol{\beta}$ are regression coefficients and $\varepsilon_k$ is an error term with mean zero. Inference is typically based on testing hypotheses of the form $H_0: \beta_j = 0$, corresponding to the absence of an association between the covariate $x_{kj}$ and the diversity measure.
...
\end{equation}
where $u_k$ captures biological variability between samples and $\varepsilon_k$ represents estimation error, whose variance is assumed to be known or estimated from the diversity estimation procedure. Inference is again based on hypotheses of the form $H_0: \beta_j = 0$, but now refers to effects on the underlying, unobserved diversity rather than on the observed estimates.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/community/overview_divnet.png}
\caption{\textbf{Model-based inference for alpha diversity.} A microbiome count matrix is combined with sample-level covariates to estimate population-level diversity measures using model-based approaches such as \texttt{breakaway} or \texttt{DivNet}. The resulting diversity estimates, together with their uncertainty, are incorporated into a regression framework to assess covariate effects on alpha diversity, yielding confidence intervals and hypothesis tests for the model parameters.}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/community/overview_beta_diversity.png}
\caption{\textbf{Overview of between-sample dissimilarity analysis.} Starting from the microbiome count matrix, pairwise dissimilarities between samples are computed to obtain a distance or dissimilarity matrix that summarizes variation in community composition across samples. This matrix can subsequently be used for ordination methods that visualize similarities between samples in a low-dimensional space and for comparative analyses testing whether microbial community composition is associated with group membership or other sample-level covariates.}
...
The Bray--Curtis dissimilarity index, originally introduced by \citet{bray1957ordination}, is one of the most widely used measures for comparing ecological community profiles. For two samples $k$ and $l$ with taxon profiles $x_k=(x_{k1},\ldots,x_{kp})^\top$ and $x_l=(x_{l1},\ldots,x_{lp})^\top$, it is defined as
\begin{equation}
d_{\mathrm{BC}}(x_k,x_l)=\frac{\sum_{i=1}^p |x_{ki}-x_{li}|}{\sum_{i=1}^p (x_{ki}+x_{li})}.
\end{equation}
If $x_k$ and $x_l$ are relative abundance vectors with equal total sum, this expression can also be written as
\begin{equation}
d_{\mathrm{BC}}(x_k,x_l)=\frac{1}{2}\sum_{i=1}^p |x_{ki}-x_{li}|,
\end{equation}
...
\begin{equation}
d_{\mathrm{E}}(x_k,x_l)=\left\| x_k - x_l\right\|_2=\sqrt{\sum_{i=1}^p (x_{ki}-x_{li})^2}.
...
\begin{equation}
d_{\mathrm{Ait}}(x_k,x_l)=\left\|\operatorname{CLR}(x_k)-\operatorname{CLR}(x_l)\right\|_2.
...
The direct application of the standard CLR transformation is complicated by the prevalence of zeros in microbiome count data, because logarithms and geometric means are not defined for zero components. In the presence of zeros, the Aitchison distance in Equation~\eqref{eq:3community:aitchison_dist} is therefore not directly defined and requires a preceding zero-handling step. In this thesis, Aitchison distances for sparse microbiome data are computed using the preprocessing strategy described in Section~\ref{sec:2micro:preprocessing_choices_thesis}: count data are first brought to a common compositional scale, and zeros are then handled by multiplicative replacement unless another strategy is explicitly motivated by the analysis. Thus, $x_k$ and $x_l$ in Equation~\eqref{eq:3community:aitchison_dist} are replaced by their normalized and multiplicatively replaced versions $x_k^\ast$ and $x_l^\ast$. The resulting distance is conditional on the chosen filtering, normalization, and zero-replacement strategy. The use of multiplicative replacement is motivated by its preservation of ratios among originally non-zero components, which is important because differences between CLR coordinates are pairwise log-ratios. At the same time, this preprocessing does not eliminate the influence of zeros from the analysis. Instead it provides a coherent and transparent way to obtain a strictly positive composition on which the CLR transformation can be evaluated.
...
A widely used ordination method for distance-based microbiome analysis is principal coordinates analysis (PCoA), also known as classical multidimensional scaling \citep{gower1966some,legendre2012numerical}. PCoA first converts the dissimilarity matrix into squared dissimilarities and then removes row and column means by double-centering. This yields a matrix that can be interpreted as an inner-product matrix whenever the dissimilarities are Euclidean distances. Let $D^{(2)}$ denote the matrix with entries $d_{kl}^2$. Double-centering can be written compactly as
\begin{equation}
B=-\frac{1}{2}HD^{(2)}H,
\end{equation}
where $H=I_n-n^{-1}\mathbf{1}\mathbf{1}^\top$ is the centering matrix.
An eigendecomposition of $B$ yields ordination axes and corresponding eigenvalues. If $\lambda_1\geq\lambda_2\geq\cdots$ denote the positive eigenvalues and $u_1,u_2,\ldots$ the associated eigenvectors, the coordinates of the samples on the first $q$ axes are given by
\begin{equation}
Y_q=(u_1,\ldots,u_q)\operatorname{diag}(\sqrt{\lambda_1},\ldots,\sqrt{\lambda_q}).
\end{equation}
...
Ordination provides a visual summary of between-sample dissimilarities, but formal assessment of group differences requires statistical testing. A common approach for testing differences in beta diversity is permutational multivariate analysis of variance (PERMANOVA), originally proposed by \citet{anderson2001new} and widely used for distance-based inference in ecological community analysis \citep{anderson2013permanova,legendre2012numerical}. PERMANOVA tests whether the centroids of predefined groups differ in the multivariate space induced by a chosen dissimilarity measure. It is therefore a natural inferential counterpart to the distance-based summaries and ordination plots introduced above.
...
Let $D=(d_{kl})$ denote a dissimilarity matrix between samples and let $g_k\in\{1,\ldots,G\}$ denote the group assignment of sample $k$. For a group $g$, the centroid $c_g$ represents the central position of all samples with $g_k=g$ in the multivariate space associated with the chosen dissimilarity \citep{anderson2006distance}. The dispersion of sample $k$ is defined as its distance to the centroid of its group,
\begin{equation}
\delta_k=d(x_k,c_{g_k}).
\end{equation}
The average community dispersion in group $g$ is then given by
\begin{equation}
\Delta_g=\frac{1}{n_g}\sum_{k:g_k=g} d(x_k,c_g),
\end{equation}
...
Community dispersion captures a different aspect of group structure than centroid location. A group with larger dispersion does not necessarily have a different average community composition, but may exhibit greater between-individual variability. In microbiome studies, such increased heterogeneity can indicate less stable or more variable community structures across individuals. At the same time, dispersion is important for the interpretation of PERMANOVA: if groups differ in their within-group dispersion, a significant PERMANOVA result may reflect heterogeneity differences rather than, or in addition to, differences in group centroids \citep{anderson2006distance,warton2012distance}. Reporting PERMDISP alongside PERMANOVA therefore helps distinguish centroid differences from heterogeneity differences and supports a more careful interpretation of distance-based microbiome comparisons.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{figures/community/overview_regression.png}
\caption{\textbf{Overview of regression models for compositional responses.} After preprocessing and CLR transformation, the microbial composition of each sample is represented by a multivariate response vector. Sample-level covariates are related to these transformed profiles through a multivariate regression model. The coefficient matrix contains taxon-specific effects on the transformed scale, whereas inference in this thesis focuses on global covariate effects across the complete microbial profile using permutation-based testing.}
...
\begin{equation}
\mathbf{y}_k = \boldsymbol{\beta}_0 + \sum_{j=1}^r w_{kj}\boldsymbol{\beta}_j + \boldsymbol{\varepsilon}_k,
\end{equation}
where $\mathbf{w}_k=(w_{k1},\ldots,w_{kr})^\top$ denotes the vector of $r$ sample-level covariates for sample $k$, $\boldsymbol{\beta}_0\in\mathbb{R}^p$ is an intercept vector, $\boldsymbol{\beta}_j\in\mathbb{R}^p$ denotes the multivariate effect of covariate $j$ across the CLR coordinates, and $\boldsymbol{\varepsilon}_k\in\mathbb{R}^p$ denotes the residual vector. Equivalently, the coefficient vectors can be collected in the coefficient matrix $\mathbf{B}=(\boldsymbol{\beta}_1,\ldots,\boldsymbol{\beta}_r)^\top\in\mathbb{R}^{r\times p}$, yielding the compact representation
\begin{equation}
\mathbf{y}_k = \boldsymbol{\beta}_0 + \mathbf{B}^\top \mathbf{w}_k + \boldsymbol{\varepsilon}_k.
\end{equation}
In this representation, row $j$ of $\mathbf{B}$ contains the coefficients describing the effect of covariate $j$ across all CLR coordinates, whereas column $i$ contains the coefficients associated with CLR coordinate $i$ across all covariates. 

The coefficient interpretation follows the CLR scale on which the response is modeled. A positive coefficient for CLR coordinate $i$ indicates that this coordinate tends to be larger for higher values of the corresponding covariate, conditional on the other covariates included in the model. Since CLR coordinates express each taxon relative to the sample-wise geometric mean, coefficients describe changes in the relative position of taxa within the microbial composition. They should therefore be understood as effects on the log-ratio representation of the community rather than as marginal effects on absolute or raw relative abundances. 

For the purposes of this chapter, the main inferential target is not an individual coefficient but the covariate-specific effect vector $\boldsymbol{\beta}_j$. Testing whether $\boldsymbol{\beta}_j=\mathbf{0}$ asks whether covariate $j$ is associated with the microbial composition as a whole. This distinguishes compositional-response regression from feature-level regression, where the primary focus lies on taxon-specific effects. At the same time, inference for the coefficient matrix $\mathbf{B}$ is non-trivial because microbiome data are high-dimensional and the CLR coordinates are statistically dependent. The question of how covariate effects are tested is therefore considered separately in the next subsection.
...
\end{equation}
Here, $\mathrm{RSS}_{\mathrm{full}}$ denotes the residual sum of squares of the full multivariate model and $\mathrm{RSS}_{\mathrm{red},j}$ denotes the residual sum of squares of the model without covariate $j$. Larger values of $t_j$ indicate that the covariate explains more variation in the CLR-transformed microbial profile. This construction is analogous to comparing nested regression models, but the null distribution is obtained empirically rather than from a parametric approximation.

The empirical permutation $p$-value is then computed by comparing the observed statistic $t_{j,\mathrm{obs}}$ with the statistics $t_{jb}^\ast$, $b=1,\ldots,B$, obtained from permuted data:
\begin{equation}
p_{\mathrm{emp},j}=\frac{1+\sum_{b=1}^B I(t_{jb}^\ast\geq t_{j,\mathrm{obs}})}{1+B}.
\end{equation}
...
In the PASTURE application in Chapter~\ref{ch:application}, compositional-response regression is used as an additional community-level analysis for the selected birth-related covariates. The focus is on the global covariate tests $H_0:\boldsymbol{\beta}_j=\mathbf{0}$, not on interpreting individual taxon-specific coefficients as separate feature-level findings. Since the tests are permutation-based, we also evaluate whether the resulting permutation $p$-values can be refined using the \texttt{permApprox} workflow introduced in Chapter~\ref{ch:permapprox}. This allows the regression-based analysis to be compared with the distance-based PERMANOVA results while retaining the same inferential framework for limited permutation settings.
...
A natural test statistic for this hypothesis compares the two sample mean vectors in CLR space. Let $\bar{y}^{(g)}=n_g^{-1}\sum_{k=1}^{n_g}y_k^{(g)}$ denote the sample mean CLR vector in group $g$, and let $\hat{\gamma}_{ii}$ denote the pooled estimate of the variance of CLR coordinate $i$. For each taxon $i$, the coordinate-wise contribution to the test statistic is defined as
\begin{equation}
m_i = \frac{n_1n_2}{n_1+n_2}\frac{\left(\bar{y}_i^{(1)}-\bar{y}_i^{(2)}\right)^2}{\hat{\gamma}_{ii}}.
...
\end{equation}
Thus, the procedure first evaluates standardized CLR mean differences separately for all taxa and then summarizes the largest contribution in a single global statistic. This maximum-type construction is motivated by sparse alternatives, where only a small number of taxa may differ between groups. In such settings, statistics that aggregate many small contributions may dilute the signal, whereas a maximum statistic is sensitive to the largest standardized deviation. At the same time, the final inferential target remains global: the test does not ask which taxon differs, but whether at least one component of the relative microbial composition differs sufficiently to reject equality of mean CLR composition.
...
In microbiome applications, randomization-based inference provides an attractive alternative to asymptotic approximations. \citet{sommer2022randomizationbased} used compositional equivalence in a randomization-based causal inference framework and emphasized that a change in the observed abundance of one taxon necessarily induces changes in sequenced counts across other taxa, even when those taxa have no causal role. Their approach therefore applies a CLR transformation before testing for differences in average microbial abundance and uses a maximum-type statistic based on standardized differences between group mean CLR coordinates.

In this thesis, we use $M_n$ as a permutation test statistic rather than relying on the asymptotic approximation. Since total sum normalization, multiplicative zero replacement, and CLR transformation are applied sample-wise and independently of the group labels, the transformed data are kept fixed during the permutation procedure. Group labels are then permuted repeatedly, and the statistic $M_n^\ast$ is recomputed for each permuted labeling, including the corresponding permuted group means and pooled coordinate variances. The observed statistic is compared with the resulting permutation distribution. For $B$ permutations, the empirical permutation p-value is computed as
\begin{equation}
p_{\mathrm{emp}}=\frac{1+\sum_{b=1}^{B} I(M_{n,b}^\ast \geq M_n)}{B+1}.
\end{equation}
This formulation is aligned with the general inferential strategy of this thesis and avoids relying on large-sample approximations whose accuracy may be difficult to assess in sparse, high-dimensional microbiome data. At the same time, it inherits the finite-resolution limitation of permutation testing: with a limited number of permutations, attainable p-values lie on a discrete grid. Therefore, small empirical p-values from compositional equivalence testing are refined using the \texttt{permApprox} framework developed in Chapter~\ref{ch:permapprox}. In this setting, $M_n$ is treated as the observed right-tailed test statistic and the permuted values $M_{n,1}^\ast,\ldots,M_{n,B}^\ast$ define the permutation distribution used for tail approximation.

Within the organization of this thesis, compositional equivalence serves as a final community-level comparison that is close to feature-level inference. In contrast to within-sample diversity, it uses the full transformed microbial profile rather than reducing each sample to a scalar ecological summary. In contrast to PERMANOVA, it is not based on pairwise sample dissimilarities, but on differences between group mean compositions in CLR space. In contrast to feature-level differential abundance analysis, it does not yield one hypothesis test per taxon, but combines taxon-wise CLR contrasts into a single global maximum-type test. In the PASTURE application, this makes compositional equivalence a useful additional analysis for selected two-group contrasts identified in the preceding community-level analyses. A significant result indicates evidence that the groups differ in their relative microbial composition, with particular sensitivity to sparse shifts in mean CLR coordinates. It does not, by itself, identify the taxa responsible for the difference and does not imply a difference in absolute microbial load. Conversely, a non-significant result should be interpreted as a lack of evidence against equality of mean CLR composition under the chosen preprocessing strategy and test statistic, not as proof of identical microbial communities.
