...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../ch03_community_level/figures/overview_alpha.pdf}
...
\end{equation}
where $n_0^{(k)}$ denotes the number of unobserved taxa. The estimation problem therefore reduces to predicting $n_0^{(k)}$. Here, the notion of an unobserved taxon is defined relative to the feature universe represented by the sequencing and preprocessing workflow. It is not an estimate of all microbial taxa that could in principle occur in the sampled environment.
...
\end{equation}
where $\boldsymbol{\beta}$ are regression coefficients and $\varepsilon_k$ is an error term with conditional mean zero given the covariates. Inference is typically based on testing hypotheses of the form $H_0: \beta_h = 0$, corresponding to the absence of an association between the covariate $x_{kh}$ and the diversity measure.
