...
These targets represent increasing structural complexity, ranging from aggregated summaries to representations of the relationships between taxa.

Orthogonal to the analysis target is the \emph{mode of analysis}. 
At each target, one may either characterize a single population or compare two or more groups. Single-population analysis aims to describe properties within a cohort, whereas comparative analysis assesses differences between groups defined by environmental, clinical, or experimental factors. Importantly, comparison is not an additional hierarchical level but a mode that applies uniformly across all analysis targets. In this thesis, we focus on the comparison between two groups.

Figure~\ref{fig:levels_inference} visualizes this conceptual organization. 
The three columns represent the analysis targets: community-level summaries, feature-level measures, and association networks. 
A more detailed overview of the three targets is given below.
The upper row in the figure corresponds to single-population analysis, while the lower row represents comparative analysis. 
Each panel therefore denotes a specific combination of analysis target and analysis mode. In addition, the figure highlights permutation testing as a cross-cutting tool for comparative analyses (Chapter~\ref{ch:permapprox}) and the integration of all components in a real-data application (Chapter~\ref{ch:application}).
The numbers indicate the chapters in which the respective components are addressed.
...
As with the other analysis targets, network-based analyses can be performed within a single population or comparatively across groups, for example by assessing differences in network topology or the edges itself.
...
Permutation testing provides a general and flexible approach for hypothesis testing in such situations. 
Instead of relying on parametric assumptions, permutation tests approximate the null distribution of a test statistic by repeatedly recomputing it under rearrangements of the data that preserve the null hypothesis. 
This approach only requires exchangeability and is therefore applicable in a wide range of settings where classical distributional assumptions are violated.

Importantly, the relevance of permutation testing extends across all analysis targets considered in this thesis.

For community-level summaries, many commonly used quantities such as alpha and beta diversity measures do not follow simple parametric distributions.
Group comparisons are therefore often based on nonparametric procedures, including permutation-based tests such as PERMANOVA, which assess differences in multivariate community composition without requiring distributional assumptions.

At the feature-level, differential abundance and distribution analyses involve statistics that are affected by data transformations needed to account for the typical data characteristics.
Consequently, the assumptions of parametric tests are often not met.
...
Permutation procedures are therefore widely used to assess differences in network structure, both at the level of individual edges and for global network summaries.
...
Over the past decade, association networks have become a central tool for exploring relationships between microbial taxa and are now routinely included in microbiome analysis pipelines. 
At the same time, the literature reveals a lack of standardization, with microbial networks being constructed in many different ways and reflecting a wide range of methodological choices. 
...
For each of these steps, a variety of methods exists.
Normalization strategies differ in their treatment of library size and compositional effects \citep{mcmurdie2014waste,mcknight2019methods,badri2020shrinkage}; compositionally aware association measures range from correlation-based approaches to conditional dependence approaches, that can even take latent influential factors into account \citep{friedman2012inferring,quinn2017propr,kurtz2015sparse,kurtz2019disentangling}; and sparsification techniques include thresholding, shrinkage, and penalized likelihood procedures \citep{schafer2005shrinkage,meinshausen2006highdimensional,friedman2008sparse}. 

These methods are typically implemented in separate software packages and designed for specific components of the workflow rather than for integrated end-to-end analysis. 
As a consequence, no unified tool exists that combines these components into a coherent and reproducible analysis pipeline, requiring researchers to assemble multiple methods in an ad hoc manner and limiting accessibility, especially for non-expert users.
...
This framework is implemented in the \texttt{NetCoMi} \texttt{R} package \citep{peschel2025netcomi} and was introduced in \citet{peschel2021netcomi}. 
...
In addition, the framework supports comparative analyses, enabling the construction and comparison of networks across groups in a statistically consistent manner. 
...
This leads to empirical $p$-values that lie on a coarse discrete grid, resulting in limited resolution in the tail of the distribution. 
After adjustment for multiple testing, this lack of resolution may substantially reduce statistical power.

Second, while approaches based on extreme value theory, in particular generalized Pareto distribution (GPD) approximations, have been proposed to address this limitation, their practical application raises additional challenges. 
In preliminary investigations, existing GPD-based methods were found to produce zero $p$-values caused by bounded support of the fitted GPD. 
Moreover, GPD-based approximation itself involves several methodological choices, including threshold selection and parameter estimation, for which a variety of approaches exists without clear guidance on their suitability in the context of permutation testing.
...
Its performance is systematically evaluated through extensive simulation studies, demonstrating accurate and stable $p$-value approximation under limited permutation budgets.
...
The third contribution of this thesis is an integrated real-data analysis that combines the different analysis targets within a coherent workflow. 
By jointly considering community-level summaries, feature-level measures, and association networks, it illustrates how consistent methodological choices across analysis targets lead to more coherent and interpretable results. 
... (add when chapter is finished)
...
$x_{ki}$ & Observed read count of taxon $i$ in sample $k$ \\
$X \in \mathbb{N}_0^{n \times p}$ & Microbiome count matrix \\
$m_k = \sum_{i=1}^p x_{ki}$ & Library size (sequencing depth) of sample $k$ \\
$\mathcal{S}^{p-1}$ & $(p-1)$-dimensional simplex \\
$r_{ij}$ & Association measure between taxa $i$ and $j$ \\
$a_{ij}$ & Entry of the adjacency matrix \\
$d_{ij}$ & Dissimilarity between taxa or samples (context-specific) \\
$M$ & Number of edges in a network, $M = \frac{p(p-1)}{2}$ \\
$m$ & Number of hypothesis tests \\
$T_{\mathrm{obs}}^{(j)}$ & Observed test statistic for test $j$ \\
$T_{b}^{*(j)}$ & Permuted test statistic (permutation $b$) for test $j$ \\
