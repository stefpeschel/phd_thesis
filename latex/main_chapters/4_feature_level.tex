...
A basic summary at feature level is the abundance of each taxon. Depending on the analysis goal, abundance can be summarized by total read count, mean relative abundance, median relative abundance, or a transformed value such as a CLR coordinate. Total abundance highlights taxa that contribute strongly to the overall sequencing output, whereas mean or median relative abundance describes the typical contribution of a taxon within samples. However, these quantities can behave quite differently in sparse microbiome data. A taxon with high total abundance may occur in only a few samples, while a taxon with moderate abundance may be observed consistently across many samples. Abundance summaries should therefore usually be considered together with prevalence.
...
The feature-wise test statistic is then computed from the rarefied counts. In the two-group setting, let $Y_i \in \{0,1\}$ denote the group label of sample $i$. DACOMP uses the Wilcoxon rank-sum statistic
\begin{equation}
\label{eq:4feature:dacomp_test_statistic}
T_j = \sum_{i:Y_i = 1} \operatorname{rank}(Z_{ij})
\end{equation}
...
H_{0j}: \text{taxon } j \text{ is not differentially abundant relative to the reference set } \mathcal{B}.
\end{equation}
...
\end{equation}
This distance has an intuitive interpretation as the amount of transport required to transform one distribution into the other. Moreover, in one dimension, the squared 2-Wasserstein distance admits a decomposition into location, size, and shape components. The location component corresponds to differences in means, the size component to differences in standard deviations, and the shape component to remaining distributional differences after centering and scaling. This decomposition is useful because it provides more insight into the nature of a detected distributional difference than a single global p-value alone.
...
\end{equation}
Under the classical Fisher combination framework for two p-values, $C_i$ is compared with a chi-squared distribution with four degrees of freedom, leading to a combined p-value
\begin{equation}
