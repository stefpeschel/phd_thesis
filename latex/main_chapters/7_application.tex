...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/overview_seq-depth-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/application/filter_prevalence_asv-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.75\textwidth]{figures/application/filter_zero_fraction_plot-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/expl_rel_to_clr_histogram_comparison-1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/application/alpha_observed-vs-predicted-richness-libsize-combined-1.pdf}
...
To understand this behavior, it is instructive to examine the frequency count structure underlying the breakaway estimator. Breakaway relies on the number of rare taxa, in particular singletons (taxa observed exactly once), to estimate the number of unobserved taxa. Figure~\ref{fig:7app:alpha_frequency_counts} shows the mean number of taxa per sample as a function of their observed frequency. Notably, the number of singletons is exactly zero across samples. As a consequence, the ratio-based estimators used by breakaway collapse to the observed richness, since there is no empirical evidence for unseen taxa. This explains the near identity between observed and breakaway richness in Figure~\ref{fig:7app:alpha_obs_vs_breakaway}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/application/alpha_freq_spectrum_genus-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/application/alpha_breast_excl_2m-1.pdf}
...
For each cohort variable and diversity measure, the Kruskal--Wallis test statistic was recomputed under random permutations of the group labels to generate an empirical null distribution for the overall group effect. The test statistic was computed directly from the group-wise rank sums using the formulation introduced in Section~\ref{sec:3community:within:testing}. Permutation-based p-values were then obtained as the proportion of permuted statistics greater than or equal to the observed statistic according to Equation~\eqref{eq:6perm:empirical_pvalue}. The comparison in this subsection focuses on the overall tests reported in Table~\ref{tab:7app:perm_results}. The pairwise Wilcoxon annotations shown in Figure~\ref{fig:7app:alpha_div_excl_bf} and Appendix~\ref{sec:app:7app:alpha_remaining} are based on asymptotic Wilcoxon rank-sum tests.
...
The resulting p-values are summarized in Table~\ref{tab:7app:perm_results}. Overall, the empirical permutation p-values closely match the asymptotic p-values and typically agree up to the same order of magnitude, even for very small p-values. This indicates that the assumptions underlying the asymptotic approximation are well satisfied in the present setting and that permutation testing would not have been necessary for obtaining reliable inference.

Nevertheless, this setting provides a useful opportunity to evaluate the behavior of the \texttt{permApprox} framework in a low-dimensional setting. The empirical permutation p-values are bounded below by $10^{-6}$ due to the finite number of permutations, whereas \texttt{permApprox} further refines the tail probabilities by fitting a generalized Pareto distribution to the upper tail of the permutation distribution under the support constraint proposed in Section~\ref{sec:6perm:constraint}. The refinement was performed using the default settings of the package, except that the number of starting exceedances was reduced to 5000 permutations (corresponding to 0.5\% of all permutations) to reduce runtime.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/application/alpha_betta_forest_combined-1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/application/alpha_cesarean_breastfeed_confounding-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/beta_PCoA_colored-5.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/beta_permanova_permapprox_perm_dist_sel-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/beta_beta_centroid_dist_sel-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/application/regression_coefficient_heatmap_top20-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.85\textwidth]{figures/application/comp_equi_taxon_contrast_top10_selected-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/expl_prevalence_abundance-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/expl_relative_abundance_barplot-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/expl_selected_taxon_violin-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{figures/application/DA_dacomp_reference_score_plot-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/DA_permutation_distribution_plot-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/DA_rarefied_count_boxplots_with_zeros-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/DD_pvalues_complete_clr_waddr_permapprox_constrained_only-1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/DD_volcano_two_stage_one_stage_native-2.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/DD_selected_taxa_complete_clr_distribution_diagnostics-2.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/net_single_association_histrogram-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/application/net_single_net_plot_col_phyla-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/application/net_single_degree_distribution-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/application/net_single_gcm-1.pdf}
...
Throughout this subsection, the network constructed in Section~\ref{sec:7app:network} is used as the baseline network. To facilitate visual comparison, all alternative networks use the same node layout and labeling as the baseline network whenever the corresponding genera are retained.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/combined_networks_prevalence.pdf}
...
Finally, the modified EM alr-algorithm described in Section~\ref{sec:2micro:zeros} was applied. This procedure imputes rounded zeros through an expectation-maximization algorithm on additive log-ratio coordinates and explicitly incorporates the estimated covariance structure. In the present data set, however, it produced a markedly denser network and retained or increased several associations among low-abundance genera, as shown in the right panel of Figure~\ref{fig:7app:network:combined_zero_repl}. Thus, accounting for multivariate dependence during imputation does not necessarily reduce sensitivity to sparse taxa and may itself induce substantial changes in the inferred network.

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/combined_networks_zero_repl.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/combined_networks_association.pdf}
...
The preceding analyses demonstrated that the estimated network structure changes substantially when prevalence filtering, zero replacement, or association estimation are modified. To summarize the resulting edge-set overlap, UpSet plots were generated separately for the three types of sensitivity analysis. Figure~\ref{fig:7app:network:upset_association} shows the comparison across association-estimation methods. The corresponding overlap summaries for prevalence filtering and zero replacement are provided in the thesis repository.
...
\midrule
\emph{Bacteroides} & \emph{Parabacteroides} & $0.20$ & $0.19$ & $0.26$ & $0.220$ \\
\emph{Rothia} & \emph{Streptococcus} & $0.13$ & $0.13$ & $0.16$ & $0.142$ \\
\emph{Streptococcus} & \emph{Veillonella} & $0.11$ & $0.11$ & $0.11$ & $0.108$ \\
\emph{Collinsella} & \emph{Senegalimassilia} & $0.08$ & $0.09$ & $0.13$ & $0.103$ \\
\emph{Staphylococcus} & \emph{Streptococcus} & $0.08$ & $0.10$ & $0.12$ & $0.102$ \\
\emph{Gemella} & \emph{Streptococcus} & $0.05$ & $0.05$ & $0.05$ & $0.049$ \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/combined_networks.pdf}
...
non-EBF & $\mathbf{0.983}$ & $\mathbf{0.667}$ & $\mathbf{0.938}$ & $0.145$ & $0.068$ & $0.128$ & $0.068$ \\
EBF & $0.513$ & $0.487$ & $0.892$ & $\mathbf{0.265}$ & $\mathbf{0.650}$ & $\mathbf{0.214}$ & $\mathbf{0.402}$ \\
\addlinespace
\multicolumn{8}{l}{\textit{Edge density}} \\
non-EBF & $\mathbf{0.033}$ & $\mathbf{0.055}$ & $0.041$ & $0.008$ & $0.005$ & $0.006$ & $0.004$ \\
EBF & $0.014$ & $0.028$ & $\mathbf{0.046}$ & $\mathbf{0.012}$ & $\mathbf{0.023}$ & $\mathbf{0.010}$ & $\mathbf{0.017}$ \\
\addlinespace
\multicolumn{8}{l}{\textit{Natural connectivity}} \\
non-EBF & $\mathbf{0.011}$ & $\mathbf{0.014}$ & $0.018$ & $0.010$ & $0.009$ & $0.010$ & $0.009$ \\
EBF & $0.010$ & $0.011$ & $\mathbf{0.019}$ & $\mathbf{0.012}$ & $\mathbf{0.011}$ & $\mathbf{0.012}$ & $\mathbf{0.011}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/application/net_thesis_ebf_network_plot_same_layout-1.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/application/net_thesis_ebf_degree_distribution_side_by_side-1.pdf}
...
The graphlet correlation distance (GCD) provides a complementary comparison of the local topological organization of the two networks. In contrast to the preceding global summary measures, it quantifies the overall discrepancy between the graphlet correlation matrices (GCMs) of the two networks. The corresponding output is shown below. Appendix Figure~\ref{fig:app:7app:net_compare_gcm} shows the two GCM heatmaps and that for the GCD, including significance codes resulting from Fisher's z tests.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{figures/application/net_diffnet_raw_pvalue_comparison-1.pdf}
\caption{\textbf{Comparison of empirical and \texttt{permApprox}-refined p-values for differential associations.} Each point represents one tested genus pair. The y-axis shows the unadjusted p-value, and point colors indicate whether the edge was significant after Benjamini--Hochberg adjustment. Because the empirical permutation analysis is based on $999$ permutations, empirical p-values are bounded below by $0.001$. The \texttt{permApprox} refinement yields continuous tail p-values below this bound. Yellow points indicate genus pairs for which the unconstrained and constrained \texttt{permApprox} p-values were nearly identical, whereas other colors highlight more noticeable differences between the two approximations.}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{figures/application/net_thesis_ebf_diffnet_plot_same_layout-1.pdf}
