...
.wassersteinTestSp <- function(x, y, permnum=10000){
    ...
    if (length(x) !=0 & length(y) != 0){
        ...
        if (num.extr < 10) {
            tryCatch({
                    res <- .gpdFittedPValue(value.sq,
                                            wass.values.ordered)
                    assign("pvalue.wass", base::unname(res["pvalue.gpd"]), env)
                    assign("pvalue.gpdfit", base::unname(res["ad.pval"]), env)
                    assign("N.exc", base::unname(res["N.exc"]), env)
                    assign("gpd.shape", base::unname(res["shape"]), env)
                }, error=function(...) {
                    assign("pvalue.wass", pvalue.ecdf.pseudo, env)
                    assign("pvalue.gpdfit", NA, env)
                    assign("N.exc", NA, env)
                    assign("gpd.shape", NA, env)
                })       
        ...
        output <- list("d.wass"=value, "d.wass^2"=value.sq, "d.comp^2"=d.comp.sq,
                    ...
                    "wass.values" = wass.values)

    } else { output <- list("d.wass"=NA, "d.wass^2"=NA, "d.comp^2"=NA,
                         "d.comp"=NA, "location"=NA, "size"=NA,
                         "shape"=NA, "rho"=NA, "pval"=NA,
                         "p.ad.gpd"=NA, "N.exc"=NA, "gpd.shape"=NA,
